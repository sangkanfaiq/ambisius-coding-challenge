    ...
    try {
      ...
      for (const order of data) {
        if (order.table === "1") {
          ...
          const existingOrder = formattedDataOrders.find((order: any) => order.key === key);
          if (existingOrder) {
            // Perbarui quantity objek pesanan yang ada
            existingOrder.quantity = order.menu.quantity + existingOrder.quantity;
          } else {
            let value = {
              key,
